...
run() {
  ...
  # --- cloud expectations (role=All, location=Cloud, ReadWrite, GcpPubSub→Kafka) ---
  ...
  [ "$(json_field "$cloud" producerQueueType)" = "GcpPubSub" ] && ok "cloud producer=GcpPubSub" || bad "cloud producer=$(json_field "$cloud" producerQueueType)"
  [ "$(json_field "$cloud" consumerQueueType)" = "Kafka" ] && ok "cloud consumer=Kafka" || bad "cloud consumer=$(json_field "$cloud" consumerQueueType)"

  # --- onprem expectations (role=All, location=OnPrem, Sentinel, Kafka→GcpPubSub) ---
  ...
  [ "$(json_field "$onprem" producerQueueType)" = "Kafka" ] && ok "onprem producer=Kafka" || bad "onprem producer=$(json_field "$onprem" producerQueueType)"
  [ "$(json_field "$onprem" consumerQueueType)" = "GcpPubSub" ] && ok "onprem consumer=GcpPubSub" || bad "onprem consumer=$(json_field "$onprem" consumerQueueType)"
