 ...
 *   </li>
 *   <li><b>HTTP Retry with Exponential Backoff</b>：
 *       對 IOException（網路錯誤）或 5xx（伺服器暫時故障）執行最多 {@value #HTTP_MAX_ATTEMPTS} 次嘗試，
 *       重試間隔採用指數退避（100ms × 2^(attempt-1)）。4xx 等客戶端永久性錯誤不重試。
 *   </li>
 * </ul>
 */
public final class MqPackReplayer {
     ...
     * 發送至遠端 App 的 {@code POST /api/replay} 端點，並在失敗時以指數退避自動重試；
        ...
        while (attempt < HTTP_MAX_ATTEMPTS) {
            ...
            try {
                ...
                if (statusCode >= 200 && statusCode < 300) {
                    // 成功
                    if (attempt > 1) {
                        log.info("[MqPackReplayer] 第 {} 次 retry 成功: {} status={}", attempt, targetUrl, statusCode);
                    }
                    metrics.recordReplaySuccess();
                    return;
                } else if (statusCode >= 500) {
                    // 5xx：伺服器暫時性錯誤，可重試
                    log.warn("[MqPackReplayer] 遠端回傳 5xx (attempt={}/{}): {} status={}",
                            attempt, HTTP_MAX_ATTEMPTS, targetUrl, statusCode);
                    // fall through to retry logic below
                } else {
                    // 4xx 或其他：客戶端永久錯誤，不重試
                    log.warn("[MqPackReplayer] 遠端回傳非重試狀態 (attempt={}): {} status={}",
                            attempt, targetUrl, statusCode);
                    metrics.recordReplayFail();
                    return;
                }
            } catch (Exception e) {
                // IOException 或其他網路層錯誤：可重試
                log.warn("[MqPackReplayer] HTTP 請求異常 (attempt={}/{}): {}",
                        attempt, HTTP_MAX_ATTEMPTS, targetUrl, e);
            }

            // 若還有重試機會，等待退避時間
            if (attempt < HTTP_MAX_ATTEMPTS) {
                long backoffMs = HTTP_BASE_BACKOFF_MS * (1L << (attempt - 1)); // 100ms, 200ms, 400ms...
                try {
